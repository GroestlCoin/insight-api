...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Groestlsight and groestlcoind? Groestlsight is configured for:';

var BAD_GEN_ERROR_DB = 'Bad genesis block. Network mismatch between Groestlsight and levelDB? Groestlsight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.getBitcoinCoreVersion(function(bitcoinVersion) {
    if (bitcoinVersion > 100000 && !config.forceRPCsync) {
      info('--------------------------------------------------------------');
      info('- Groestlcoin Core version >2.1.0.6 only works with RPC sync -');
      info('- Set the env variable INSIGHT_FORCE_RPC_SYNC = 1            -');
      info('--------------------------------------------------------------');
      process.exit(1);
    } else {
      info('Groestlcoin Core version ', bitcoinVersion);
      info('Using RPC sync ');
    }
  });
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if (self.blockChainHeight === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on groeslcoind. Please reset Groestlsight DB. Tip was:' + tip)
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at groestlcoind');

  if (self.blockExtractor) {
    info('groestlcoind dataDir configured...importing blocks from .dat files');
