...
var home = process.env.INSIGHT_DB || (getUserHome() + '/.groestlsight');
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\groestlcoin\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/groestlcoin/';
  if (isLinux) dataDir = process.env.HOME + '/.groestlcoin/';
}
